...
var _Page2 = _interopRequireWildcard(_Page);
...
var page = new _Page2['default'](pageIndex);
page.test(_map2['default'].map);

//when at the end of the page : load next page + dependencies (js, spritesheets, images, topjpson) + n+2 page intro

page.on('load:gps', function (names, topoJsonData) {
	return _map2['default'].initGPS(names, topoJsonData);
});

page.on('scrollblock:scroll', function (block) {
...
page.on('scrollblock:enter', function (block, down) {
	if (block.data.gpstrace) {
		var progress = updateGPS(block, down ? -1 : 1);
		_map2['default'].updateGPSStatuses(block.data.gpstrace);

		if (block.data.timelapse) {
			page.startTimelapse();
		}
	}
	if (block.data.mapview) _map2['default'].setView(block.data.mapview);

	if (block.data.geocodedimages) _map2['default'].showGeocodedImages(block.el);
});

page.on('scrollblock:leave', function (block, down) {
...
function updateGPS(block, startOrEnd) {
	...
	_map2['default'].updateGPS(block.data.gpstrace, r, !_.isUndefined(block.data.gpstracefollow));
	return r;
}
...
},{"./Page":2,"./map":4,"jquery":8,"leaflet/dist/leaflet-src":10,"underscore":15}],2:[function(require,module,exports){
'use strict';
...
var _Timelapse2 = _interopRequireWildcard(_Timelapse);
	...
	_createClass(Page, [{
		...
		value: function setupFromDom() {
			...
			$('[data-trigger]').each(function (i, el) {
				...
				_.each(scrollBlock.data, function (v, k) {
				...
				_this.data.scrollBlocks.push(scrollBlock);
			});

			this.data.scrollBlocks = _.sortBy(this.data.scrollBlocks, function (s) {
				return s.start;
			});

			//get all gps traces for preloading
			this.data.gpstrace = [];
			$('[data-gpstrace]').each(function (i, el) {
				_this.data.gpstrace.push($(el).data('gpstrace'));
			});
		...
		value: function loadData() {
			...
			var gpstracesPromises = this.data.gpstrace.map(function (gps) {
				return utils.load('./data/' + _this2.data.index + '/' + gps + '.topojson');
			});

			Promise.all(gpstracesPromises).then(function (data) {

				_this2.trigger('load:gps', _this2.data.gpstrace, data.slice(0, gpstracesPromises.length));
		...
		value: function _start() {
			...
			if (this.data.timelapse) this._timelapse = new _Timelapse2['default']();
		...
		key: 'startTimelapse',
		value: function startTimelapse() {
			$('.js-imgprv').show();
		}
	}, {
		key: 'updateTimelapse',
		value: function updateTimelapse(block, r) {
			this._timelapse.update(r);
		}
	}, {
		key: 'test',
		value: function test(map) {
			this.map = map;
			return;
...
},{"./Timelapse":3,"./utils":5,"backbone":6,"jquery":8,"underscore":15}],3:[function(require,module,exports){
'use strict';
    ...
    _createClass(Timelapse, [{
        ...
        value: function update(r) {
            ...
            if (!_.isEqual(this.currentSpriteSheetCoords, spriteSheetCoords)) {
                ...
                this.showMediumTimeout = setTimeout(function () {
                    console.log(imgIndex);
                    _this.mediumImage = document.createElement('img');
                    _this.mediumImage.addEventListener('load', _this.onMediumImageLoaded.bind(_this));
                    _this.mediumImage.setAttribute('src', 'data/6/timelapse/medium/' + imgIndex + '.jpg');
                    _this.mediumImage.setAttribute('class', 'medium');
                    _this.timelapseMedium.appendChild(_this.mediumImage);
                }, 500);
            }
        ...
        key: 'onMediumImageLoaded',
        value: function onMediumImageLoaded() {
...
var MapAPI = {
	initGPS: function initGPS(names, topoJsonData) {

		for (var i = 0; i < topoJsonData.length; i++) {
			var topo = topoJsonData[i];
			var geo = topojson.feature(topo, topo.objects.stdin);
			var coords = geo.features[0].geometry.coordinates.map(function (coord) {
	...
	updateGPS: function updateGPS(name, r, follow) {
		...
		if (follow) {
			...
			var polygon = turf_polygon([meanCoords]);
			var centroid = turf_centroid(polygon);
			map.panTo(centroid.geometry.coordinates, { animate: false });
		}
	},

	updateGPSStatuses: function updateGPSStatuses(currentlyEntering) {
	...
	setView: function setView(coords) {
		map.setView([coords[1], coords[2]], coords[0]);
	},

	showGeocodedImages: function showGeocodedImages(el) {

		var images = [];

		el.find('.geocodedImages img').each(function (index, img) {
...
},{"./utils":5,"formatcoords":7,"jquery":8,"leaflet-hash":9,"leaflet/dist/leaflet-src":10,"turf-centroid":11,"turf-point":16,"turf-polygon":14,"underscore":15}],5:[function(require,module,exports){
'use strict';
...
exports.load = load;
exports.parseDataAttrFloats = parseDataAttrFloats;
exports.swapLL = swapLL;
...
function parseDataAttrFloats(data, keys) {
    var _ = require('underscore');
...
},{"turf-meta":12,"turf-point":13}],12:[function(require,module,exports){
/**
