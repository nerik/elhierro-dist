...
var getXHRPromise = function getXHRPromise(url) {
...
Promise.all([getXHRPromise('./data/6/0_car_topo.json'), getXHRPromise('./data/6/1_feet_test_topo.json')]).then(function (data) {
	//invert lat/lon manually :/

	var car0topo = data[0];
	var car0geo = topojson.feature(car0topo, car0topo.objects['0_car']);

	var car0 = car0geo.features[0].geometry.coordinates.map(function (coords) {
		return [coords[1], coords[0]];
	});
	L.polyline(car0, { color: 'red', dashArray: '10,10', lineCap: 'square', weight: 8, opacity: 0.3 }).addTo(map);

	var testTopo = data[1];
	var testGeo = topojson.feature(testTopo, testTopo.objects['1_feet_test']);

	testCoords = testGeo.features[0].geometry.coordinates.map(function (coords) {
		return [coords[1], coords[0]];
	});
	testPath = L.polyline([], { color: 'red', weight: 8, opacity: 0.3 }).addTo(map);
...
var totalScroll = Math.max(body.scrollHeight, body.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight);
...
var onMediumImageLoaded = function onMediumImageLoaded() {
...
},{"leaflet/dist/leaflet-src":2,"underscore":3}],2:[function(require,module,exports){
/*
